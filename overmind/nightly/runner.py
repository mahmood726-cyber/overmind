"""Phase-4 M-2: Nightly verifier runner — heavy verification driver.

Holds the multiprocessing worker harness, the per-project verification
loop, and the daily-report writer that previously lived in
scripts/nightly_verify.py.

Phase-3 M extracted the pure helpers (paths, selection, reporting,
integrations) but left the 820-LOC _run_verification driver in place.
Phase-4 M-2 finishes the job: nightly_verify.py is now a 50-LOC shim that
sets up faulthandler / stdout / atexit and delegates here.

Behavior preservation: the function definitions below are literal moves
from scripts/nightly_verify.py. Module-level names they reference
(SKIP_PROJECTS, PROJECT_WORKER_TIMEOUTS, _atomic_write_text,
_promote_progress_to_partial_report, ...) are now imported from the
overmind.nightly submodules instead of bound in the same file. References
that previously resolved against the script's module globals now resolve
against this module's globals — same objects either way, since both
import from the same sources of truth.
"""
...
from overmind.nightly.paths import DATA_DIR, DB_PATH, REPORT_DIR
from overmind.nightly.selection import (
    SKIP_PROJECTS,
...
from overmind.nightly.reporting import (
    _atomic_write_text,
...
from overmind.nightly.integrations import (
