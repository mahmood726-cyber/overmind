...
RESTORED 2026-06-07 (PASS): the `dataextractor-9c5488b5` baseline. It was briefly
removed because `RCTExtractor_v4_8_AI.js` requires `RCTExtractor_v4_7.js` (the base
extract/flatten/getSummary), which had never been committed -> module threw on load.
FIXED by recovering the base engine verbatim from the version-matched
RCTExtractor_WebApp.html bundle (v5.1.0, lines 681-1689) + a dot-path `flatten`
(Dataextractor commit 91438dc). probe.cjs now reproduces the EXACT structural
signature the original baseline captured (version 5.1.0-AI-NMA-i18n, 7 exports, 18
engine keys, 16 methods) -> witness PASS, recovery faithful. Remaining (separate,
NOT the core engine): index.js (npm main) still needs ./qc, ./viz, ./lib, also
never committed and not recoverable from any in-repo source.
